...
public class GatewayConfig {
	...
	public RouteLocator routes(RouteLocatorBuilder builder) {
		return builder.routes()
				.route("authentication-service", r -> r.path("/**").filters(f -> f.filter(filter)).uri("lb://authentication-service"))
				.route("service-discovery", r -> r.path("/**").filters(f -> f.filter(filter)).uri("lb://service-discovery"))
				.build();
	}
}
